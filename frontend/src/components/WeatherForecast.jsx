...
// ✅ Error boundary prevents full white-screen on any future crash
class ErrorBoundary extends React.Component {
  constructor(props) { super(props); this.state = { hasError: false } }
  static getDerivedStateFromError() { return { hasError: true } }
  render() {
    if (this.state.hasError)
      return <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-600">⚠️ Something went wrong loading the forecast. Please refresh.</div>
  ...
  return (
    <ErrorBoundary>
      <div className="space-y-5">
        ...
        {/* ✅ ALL forecast rendering safely inside one null guard */}
        {forecast && !loading && (
          <>
            {/* Fallback banner — only shown when live data unavailable */}
